...
import { Box, IconButton, Button, Typography, Avatar, TextField } from '@mui/material';
import { ArrowBack, Share, CheckCircle, Send } from '@mui/icons-material';
import { PostBody } from '@/components/blog/PostBody';
import { draftPost, jonny, maya, seededComments } from '@/mocks/blogDemo';
import type { BlogComment } from '@/mocks/blogDemo';
import { ShareRecommendationDialog } from './ShareRecommendationDialog';

interface FeedPostViewScreenProps {
  onBack?: () => void;
}
...
export const FeedPostViewScreen = ({ onBack }: FeedPostViewScreenProps) => {
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: 'background.default' }}>
      <Box sx={{ px: 1, pt: 1, display: 'flex', alignItems: 'center', gap: 1, flexShrink: 0 }}>
        <IconButton size="small" onClick={onBack} aria-label="Back">
          <ArrowBack sx={{ fontSize: 18 }} />
        </IconButton>
        <Typography sx={{ fontSize: '0.72rem', fontWeight: 700 }}>Friends Feed</Typography>
      </Box>

      <Box sx={{ flex: 1, overflow: 'auto' }}>
        <PostBody post={post} author={jonny} actions={followAction} />
      ...
      <ShareRecommendationDialog open={shareOpen} onClose={() => setShareOpen(false)} />
    </Box>
  );
};
