...
export function processContactFromJSON(jsonContact: any, withIds = true): Contact {
  ...
  contactLdSetProperties.forEach(property => {
    if (jsonContact[property] && Array.isArray(jsonContact[property])) {
      const value = jsonContact[property].map((el: any) => {
        if (withIds) {
          el["@id"] = Math.random().toExponential(32);
        }
        if (el.type2) {
          el.type2 = new BasicLdSet([el.type2]);
        }
        return el;
      });
...
export const dataService = {
  getDraftContact() {
    if (!draftContact) {
      ...
      draftContact = processContactFromJSON(contactJson);
    ...
    return new Promise((resolve) => {
      setTimeout(async () => {
        try {
          ...
          const filteredContacts: Contact[] = contactsData
            // @ts-expect-error just mock
            .filter((jsonContact) => {
              // Check if any field has the specified source
              const hasSource = (
                // @ts-expect-error just mock
                (jsonContact.name && jsonContact.name.some((n) => n.source === sourceId)) ||
                // @ts-expect-error just mock
                (jsonContact.email && jsonContact.email.some((e) => e.source === sourceId)) ||
                // @ts-expect-error just mock
