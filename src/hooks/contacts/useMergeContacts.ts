import {dataService, processContactFromJSON} from "@/services/dataService.ts";
...
export function useMergeContacts(): UseMergeContactsReturn {
  ...
  const calcMergedContact = (contactsToMerge: Contact[]): Contact | null => {
    ...
    return processContactFromJSON(mergedContactJson, !isNextGraph);
  }
  ...
  const mergeContacts = async (mergingContactIds: (string)[]) => {
    ...
    try {
      const mergedContact = calcMergedContact(mergingContacts);
